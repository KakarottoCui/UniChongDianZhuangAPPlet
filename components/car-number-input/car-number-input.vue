<template>
	<u-popup v-model="isShow" mode="bottom" :closeable="true" border-radius="23" @close="closePop">
		...
		<view class="" style="height: 600rpx;">
			...
			<view class="car-input-container">
				<view class="car-input-box" v-for="(item,index) in inputList" :key="index" @click="plateInput(index)">
					<view class="car-input-item" :class="[curInput == index?'sel-item':'']">
						<view :class="curInput == index?'sel-item-line':''"></view>
						<!-- <img :src="xnyImgBase64" class="new-item-img" v-if="(maxNum-1) == index" /> -->
						{{item}}
					</view>
			...
			<view class="car-number-container" v-if="showKeyPop1">
				<view class="" style="display: flex;width: 100%;">
					<view class="plate-close " @click="closeKeyboard(true)"><text class="plate-close-btn sure">确定</text>
					</view>
					<view class="plate-close" @click="closeKeyboard(false)"><text class="plate-close-btn">关闭</text>
					</view>
				</view>
			...
			<view class="car-number-container" v-if="showKeyPop2">
				<view class="" style="display: flex;width: 100%;">
					<view class="plate-close sure" @click="closeKeyboard(true)"><text
							class="plate-close-btn sure">确定</text></view>
					<view class="plate-close" @click="closeKeyboard(false)"><text class="plate-close-btn">关闭</text>
					</view>
				</view>
		...
		methods: {
			...
			closeKeyboard(flag) {
				if (flag) {
					...
					this.$emit('numberInputResult', returnResult,true);
				}
				this.$emit("closePop", false)
				this.inputList = [" ", " ", " ", " ", " ", " ", " ", " "];
				this.curInput = 0
